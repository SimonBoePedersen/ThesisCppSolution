...
#ifndef _CubicSpline_
#define _CubicSpline_
...
// https://github.com/glentner/CubicSplineLib/blob/master/CubicSpline.hh
...
public:
	...
	CubicSpline(std::vector<T>& _x_, std::vector<T>& _y_,
		bool sorted = false);
	...
	std::vector<T> interpolate(std::vector<T>& new_x,
		const bool& fixed = false, const T& value = 0);

	T interpolate(T& new_x, const bool& fixed = false,
		const T&value = 0);

	std::vector<T> interpolate_derivative(std::vector<T>& new_x,
		const bool& fixed = false, const T& value = 0);

	T interpolate_derivative(T& new_x, const bool& fixed = false,
		const T& value = 0);
...
// primary constructor
template<class T>
CubicSpline<T>::CubicSpline(std::vector<T>& _x_, std::vector<T>& _y_,
	bool sorted) {
...
template<class T>
std::vector<T> CubicSpline<T>::interpolate(std::vector<T>& new_x,
...
template<class T>
T CubicSpline<T>::interpolate(T& new_x, const bool& fixed,
	const T& value) {
...
template<class T>
T CubicSpline<T>::interpolate_derivative(T& new_x, const bool& fixed,
	const T& value) {
	// Added by Simon Boe Pedersen
	T output;
	...
	if (new_x < x[0] || new_x > x[n]) {
		...
		if (fixed)
			...
			output = k[0] * (new_x - x[0]);

		else
			output = k[n] * (new_x - x[n]);
		...
		T t = (new_x - x[j - 1]) / (x[j] - x[j - 1]);
		T tdiff = 1 / (x[j] - x[j - 1]);
		output = (y[j] - y[j - 1] + a[j] + 3 * t * t * a[j] - 4 * t * a[j] +
			2 * t * b[j] - 3 * t * t * b[j]) * tdiff; // By chain rule
	}
...
template<class T>
inline std::vector<T> CubicSpline<T>::interpolate_derivative(std::vector<T>& new_x,
	const bool & fixed, const T& value)
{
	...
	for (long i = 0; i < new_x.size(); i++)
		output[i] = interpolate_derivative(new_x[i], fixed, value);

	return output;
}

template<typename T>
CubicSpline<double> as_double(CubicSpline<T>& self) {
	CubicSpline<double> out;
	out.a = as_double(self.a);
	out.b = as_double(self.b);
	out.c = as_double(self.c);
	out.d = as_double(self.d);
	out.c_prime = as_double(self.c_prime);
	out.d_prime = as_double(self.d_prime);
	out.k = as_double(self.k);
	out.n = self.n;
	out.x = as_double(self.x);
	out.y = as_double(self.y);
	return out;
}